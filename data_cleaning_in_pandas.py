# -*- coding: utf-8 -*-
"""Data Cleaning in Pandas.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1yi2HsgQcvyAGj5XrfmEBiFFmUQvI5Uq7
"""

import pandas as pd

df = pd.read_excel(r"/content/Customer Call List.xlsx")
df

df = df.drop_duplicates()

df

df = df.drop(columns="Not_Useful_Column")

df

df["Last_Name"].str.lstrip(".")
df["Last_Name"].str.lstrip("/")
df["Last_Name"].str.rstrip("_")

df["Last_Name"] = df["Last_Name"].str.strip("123._/")

df

df["Phone_Number"] = df["Phone_Number"].str.replace('[^a-zA-Z0-9]', '', regex = True)

df["Phone_Number"] = df["Phone_Number"].apply(lambda x: str(x))
df["Phone_Number"] = df["Phone_Number"].apply(lambda x: x[0:3] + "-" + x[3:6] + "-" + x[6:9])

df["Phone_Number"] = df["Phone_Number"].str.replace("nan--", "", regex=False)
df["Phone_Number"] = df["Phone_Number"].str.replace("Na--", "", regex=False)
df["Phone_Number"] = df["Phone_Number"].str.replace("--", "", regex=False)

df

df[["Street_Addresss", "State", "Zip_Code"]] = df["Address"].str.split(",", n=2, expand=True)

df

df['Paying Customer'] = df['Paying Customer'].str.replace('Yes', 'Y')
df['Paying Customer'] = df['Paying Customer'].str.replace('No', 'N')

df

df["Do_Not_Contact"] = df["Do_Not_Contact"].str.replace('Yes', 'Y')
df["Do_Not_Contact"] = df["Do_Not_Contact"].str.replace('No', 'N')

df

df = df.replace('N/a', '')
df = df.fillna('')

df

for x in df.index:
  if df.loc[x, "Do_Not_Contact"] == 'Y':
    df.drop(x, inplace=True)

df

# for x in df.index:
#   if df.loc[x, "Phone_Number"] == '':
#     df.drop(x, inplace=True)

# df

# Another way to drop null  values
df.dropna(subset="Phone_Number", inplace=True)
# prompt: drop every row in df that has an empty string value in the column Phone_Number
df = df[df["Phone_Number"] != ""]
df

df = df.reset_index(drop=True)
df









